...
if __name__ == '__main__':
    ...
    CLIENT = get_downloadclients(RADARR_DB, DOWNLOAD_NAME)
    if CLIENT is None:
        sys.exit(1)

    if CLIENT["username"] == "" and CLIENT["password"] == "" and CLIENT["port"] == "" and CLIENT["url"] == "":
        CLIENT = set_downloadclients(RADARR_DB, DOWNLOAD_NAME, DOWNLOAD_URL, DOWNLOAD_PORT, DOWNLOAD_USER, DOWNLOAD_PASSWORD)
        if CLIENT is None:
            sys.exit(1)
    ...
    if CLIENT["username"] != DOWNLOAD_USER or CLIENT["password"] != DOWNLOAD_PASSWORD or CLIENT["port"] != DOWNLOAD_PORT or CLIENT["url"] != DOWNLOAD_URL:
        ...
        CLIENT = update_downloadclients(RADARR_DB, DOWNLOAD_NAME, DOWNLOAD_URL, DOWNLOAD_PORT, DOWNLOAD_USER, DOWNLOAD_PASSWORD)
        if CLIENT is None:
            sys.exit(1)
